...
class GitLabMCPServer {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            try {
                switch (name) {
                    ...
                    case 'get_job_trace':
                        return await this.getJobTrace(args);
                    default:
    ...
    async listProjects(args) {
        ...
        if (args.visibility)
            params.append('visibility', args.visibility);
        if (args.owned)
    ...
    async listIssues(args) {
        ...
        if (args.author_id)
            params.append('author_id', String(args.author_id));
        if (args.search)
            params.append('search', args.search);
        params.append('per_page', String(args.per_page || 20));
    ...
    async createIssue(args) {
        ...
        if (args.assignee_ids)
            data.assignee_ids = args.assignee_ids;
    ...
    async listMergeRequests(args) {
        ...
        if (args.source_branch)
            params.append('source_branch', args.source_branch);
        if (args.assignee_id)
            params.append('assignee_id', String(args.assignee_id));
        if (args.author_id)
            params.append('author_id', String(args.author_id));
    ...
    async createMergeRequest(args) {
        ...
        if (args.reviewer_ids)
            data.reviewer_ids = args.reviewer_ids;
        if (args.labels)
            data.labels = args.labels;
        if (args.milestone_id)
            data.milestone_id = args.milestone_id;
    ...
    async listProjectBranches(args) {
        const params = new URLSearchParams();
        if (args.search)
            params.append('search', args.search);
        params.append('per_page', String(args.per_page || 20));
    ...
    async getProjectCommits(args) {
        const params = new URLSearchParams();
        if (args.ref_name)
            params.append('ref_name', args.ref_name);
        if (args.since)
    ...
    async listPipelines(args) {
        ...
        if (args.ref)
            params.append('ref', args.ref);
        if (args.sha)
            params.append('sha', args.sha);
        if (args.yaml_errors !== undefined)
            params.append('yaml_errors', String(args.yaml_errors));
        if (args.name)
        ...
        if (args.updated_before)
            params.append('updated_before', args.updated_before);
        if (args.order_by)
    ...
    async listPipelineJobs(args) {
        const params = new URLSearchParams();
        if (args.scope && args.scope.length > 0) {
            args.scope.forEach((status) => params.append('scope[]', status));
        }
        if (args.include_retried !== undefined)
            params.append('include_retried', String(args.include_retried));
    ...
    async getPipelineVariables(args) {
