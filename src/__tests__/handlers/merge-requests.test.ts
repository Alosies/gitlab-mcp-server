...
describe('MergeRequestHandlers', () => {
  ...
  beforeEach(() => {
    ...
    mergeRequestHandlers = new MergeRequestHandlers(mockClient);
  });
  ...
  describe('listMergeRequests', () => {
    ...
    it('should not include scope if not provided', async () => {
      ...
      expect(mockClient.get).toHaveBeenCalledWith('/projects/123/merge_requests?state=merged&per_page=20');
    });
  });
